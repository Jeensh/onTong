...
import { Code, Network, GitCompare, Search, CheckSquare, PackageOpen, Loader2, CircleCheck, Circle, CircleDot } from "lucide-react";
...
const NAV_ITEMS: NavItem[] = [
  { id: "code", label: "코드 분석", icon: <Code size={18} />, description: "Java 코드 파싱 및 의존성 그래프", step: 1 },
  { id: "ontology", label: "도메인 온톨로지", icon: <Network size={18} />, description: "SCOR+ISA-95 프로세스 트리", step: 2 },
...
export function ModelingSection() {
  const [activeView, setActiveView] = useState<ModelingView>("code");
  ...
  // Re-check workflow when user switches tabs (data may have changed)
  useEffect(() => {
    if (repoId) checkWorkflow(repoId);
  ...
  // Determine which step is the recommended next
