 ...
 * Composite PK: (CMP_CD, ORG_CD, PROC_CD, GRADE_CD, PROD_KIND_CD, CUSTOMER_CD).
 *
 * PROC_CD 값: SM / HR / HRF / CR / ANL1 / ANL2 / GAL / CRF (8 공정 약어).
 *
 * NOTE: PROD_KIND_CD 는 다른 테이블의 PRODUCT_TYPE_CD / PRODUCT_NAME_CD 와 동일 도메인 (품명/품종/품종코드).
 *       레거시 비표준화 그대로 보존.
 */
@Entity
...
public class SdProductivityStdJpo {
    ...
    @Id @Column(name = "PROD_KIND_CD", length = 4)
    private String prodKindCd;      // 품종코드 — 동일 의미, 레거시 비표준 컬럼명
