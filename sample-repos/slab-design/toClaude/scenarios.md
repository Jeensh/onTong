...
이 문서는 **Slab 설계 메타 툴 (온톨로지 매핑 + 영향도 분석 + 샌드박스 시뮬레이션) 의 시연 시나리오**를 정리합니다. 각 시나리오는 코드에 심어진 "드라마 DNA 시드"를 활용해 "이거 바꾸면 저기가 터진다" 또는 "겉보기에는 분리되어 있지만 도메인적으로 연결된 짝"을 시연합니다.
...
- 샌드박스: 변경 후 알고리즘 재실행 → 새 Slab 설계 결과 비교
...
- 영향받는 주문 = 그 영역에 떨어지는 Slab들 → step 6 의 단중상한이 변동
...
- 즉 한 공정 row 변경 → 알고리즘 3 군데에서 출력 변동 → 최종 Slab 결과 영향
...
- 삭제 시 IllegalStateException 던지는 (즉 Slab 설계 자체가 불가능해지는) 주문 N건
...
- 한 Slab의 결정 (단중·매수·분할수) 이 분할수 1 차이만으로 크게 변동
...
- 영향: step 9 의 매수 변동 → Slab 매수 감소
- 결과: 같은 주문이 더 적은 Slab으로 처리 가능
...
- 재고주문은 새 Slab 설계 대상 아님 (기존 재고에서 출고)
