 ...
 *
 * @author 김XX (2017-06-12 최초작성, 13 step 구조)
 * @author 정XX (2018-08-19 A-a 루프 도입 — step 8~15 분할 fallback)
 * @author 박XX (2020-01-30 21 step 으로 확장 — final width/length range 분리)
 * @author 이XX (2021-09-14 history wrapper 패턴 도입, executeWithHistory)
 *
 * 운영이슈 P-2018-0721 (2018-07-15):
 *   - 실수율 과다 시 분할수 폭주 → 무한 루프 가까운 동작
 *   - 해결: A-a 루프 max iteration = maxSplitCount → 1 강제 (정XX)
 *
 * 운영이슈 P-2019-0445 (2019-06-08):
 *   - history 적재 실패 시 main tx rollback → Slab 미생성
 *   - 해결: history 자체 @Transactional 분리 (REQUIRES_NEW) — 이XX 패치
 *
 * TODO(2022-04-01 박XX): step 14 제품단중 최대화 모드 — 현재 미구현 (보류)
 */
@Component
public class SdDesigner {
    ...
    public SDSlabEntity design(SDOrderEntity order) {
        ...
        try {
            ...
            executeWithHistory(1, "SLAB_THICKNESS",          order, slab, thicknessAction::execute);
            executeWithHistory(2, "FIRST_WIDTH_RANGE",       order, slab, widthRangeAction::execute);
            executeWithHistory(3, "FIRST_LENGTH_RANGE",      order, slab, lengthRangeAction::execute);
            executeWithHistory(4, "FIRST_WGT_RANGE",         order, slab, firstWeightAction::execute);
            executeWithHistory(5, "SECOND_WGT_LOW",          order, slab, secondWgtLowAction::execute);
            executeWithHistory(6, "SECOND_WGT_HIGH",         order, slab, secondWgtHighAction::execute);
            executeWithHistory(7, "MAX_SPLIT_COUNT",         order, slab, maxSplitCountAction::execute);

            // A-a 루프 (steps 8~15, 내부 attempts silent — 수렴 시점만 history 적재)
            runAaLoop(order, slab);
            historyAction.recordStep(order, slab.getSlabNo(), 15, "A_A_LOOP_CONVERGED",
                snapshotSlab(slab));

            // final steps 16~19 (step별 history 적재)
            executeWithHistory(16, "FINAL_WIDTH_RANGE",      order, slab, finalWidthRangeAction::execute);
            executeWithHistory(17, "FINAL_LENGTH_RANGE",     order, slab, finalLengthRangeAction::execute);
            executeWithHistory(18, "TARGET_WIDTH",           order, slab, targetWidthAction::execute);
            executeWithHistory(19, "TARGET_LENGTH",          order, slab, targetLengthAction::execute);

            // step 20: SLAB_RESULT 매수 row 저장
            List<String> savedSlabNos = slabSaveAction.execute(order, slab);
     ...
     */
    private void runAaLoop(SDOrderEntity order, SDSlabEntity slab) {
        int maxSplit = slab.getMaxSplitCountUpper();

        for (int split = maxSplit; split >= 1; split--) {
            slab.setCurrentSplitCount(split);

            if (!tryStep(order, slab, splitRangeAction::execute)) continue;
            if (!tryStep(order, slab, slabCountAction::execute)) continue;

            if (tryStep(order, slab, initialSlabWgtAction::execute)) {
                return; // step 10 YES → step 11 implicit
            }
            if (tryStep(order, slab, slabWgtRecalcAction::execute)) {
    ...
    private void executeWithHistory(int stepNo, String stepName,
    ...
    /** A-a 루프 내부 silent 시도. DG108 → false, 다른 fail → throw. */
    private boolean tryStep(SDOrderEntity order, SDSlabEntity slab, AlgorithmStep step) {
        try {
            step.run(order, slab);
            return true;
        } catch (AlgorithmException e) {
