 ...
 *   - 재고주문 일부가 신규 설계 대상으로 들어와 Slab 중복 생성
 *   - 해결: STOCK_CODE=1 일 때 즉시 fail (DG001) — 정XX 패치
 *
 * 운영이슈 P-2020-0411 (2020-05-30):
 *   - WORK_DUE 가 오늘 날짜와 동일한 경우 통과되어 당일 마감 Slab 생성
 *   - 해결: isAfter(today) 로 변경 (>= → >) — 박XX 패치 후 안정화
 */
@Component
public class SdOrderValidator {
    ...
    private ValidationResult checkWorkDue(SDOrderEntity o) {
        ...
        if (confirmed == null || confirmed.length() < 8) {
            return ValidationResult.fail(SdErrorCode.VAL_WORK_DUE,
                "확정통과공장코드 형식 오류 (8자리 필요)");
        }
        LocalDate[] dues = {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (plantChar == ' ') continue; // 비활성 공정은 점검 skip
