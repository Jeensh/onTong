    ...
    public void execute(SDOrderEntity order, SDSlabEntity slab) {
        ...
        if (edgingGroup == null) {
            throw failEdgingGroup("EDGING_GROUP 매칭 실패 (grade=" + order.getGradeCd()
                + ", product=" + order.getProductCd() + ", customer=" + order.getCustomerCd()
                + ", width=" + selectedWidth + ")");
        }
