...
`apiVersion: cert-manager.io/v1   kind: Certificate   metadata:   name: api-tls   namespace: production   spec:   secretName: api-tls-secret   issuerRef:   name: letsencrypt-prod   kind: ClusterIssuer   dnsNames:   - api.company.com   renewBefore: 720h # 만료 30일 전 자동 갱신   `

### 갱신 상태 확인

`# 인증서 상태 확인   kubectl get certificates -A      # 갱신 이벤트 확인   kubectl describe certificate api-tls -n production   `
...
2.  CSR 생성:
    ...
    `openssl req -new -newkey rsa:2048 -nodes \   -keyout company.key -out company.csr \   -subj "/CN=*.company.com/O=Company Inc"   `
