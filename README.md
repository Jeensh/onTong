...
스킬은 `wiki/_skills/` 폴더에 마크다운 파일로 정의합니다.

```markdown
---
type: skill
description: 장애 발생 시 초기 대응을 안내합니다
trigger:
  - 장애
  - 서버 다운
icon: 🚨
...
## 지시사항
사용자가 장애 상황을 설명하면 등급을 판정하고 대응 절차를 안내하세요.
...
- `trigger`: 사용자 질문에서 이 키워드가 감지되면 스킬이 자동 활성화됩니다
- `[[WikiLink]]`: 참조 문서의 내용이 AI에게 자동으로 전달됩니다
- 6개 레이어(역할/워크플로우/지시사항/체크리스트/출력형식/제한사항)를 모두 채울수록 답변 품질이 높아집니다
