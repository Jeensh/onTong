...
flowchart LR
    subgraph P0["Phase 0: 플랜 협의"]
        direction TB
        A1[요구사항 설명] --> A2[AI: 플랜 초안 작성]
        A2 --> A3["⭐ 사용자 리뷰/편집<br/>(삭제·순서변경·축소)"]
        A3 --> A4[TODO.md 확정]
    end

    subgraph P1["Phase 1: 반복 실행"]
        direction TB
        B1[CHANGES.md 확인] --> B2["코드 구현 (+ TDD)"]
        B2 --> B3[검증 + 문서 갱신]
        B3 --> B4["TODO.md [x] 체크"]
        B4 --> B5[사용자에게 보고]
    end
    ...
    subgraph P2["Phase 2: 데모-피드백"]
        direction TB
        C1[demo_guide 전달] --> C2[사용자 데모 테스트]
        C2 --> C3["피드백 → CHANGES.md"]
    end
...
```mermaid
flowchart LR
    A[AI 구현] --> B[데모 시나리오 제공]
    B --> C[사용자 검증]
    C --> D["피드백 캡처<br/>• CHANGES.md<br/>• TODO.md<br/>• demo_guide.md"]
    D --> A
```
...
```mermaid
flowchart TB
    A["1. AI: master_plan 초안 제출<br/>(스텝/태스크 표)"] --> B["2. 사용자: 표를 읽으며 편집"]
    B --> B1["태스크 삭제: 이건 지금 안 해"]
    B --> B2["순서 변경: 이걸 먼저 해야 해"]
    B --> B3["스텝 분할/병합: 너무 크다 / 합쳐도 돼"]
    B --> B4["기술 선택 변경: X 대신 Y로 하자"]
    B1 & B2 & B3 & B4 --> C["3. AI: 피드백 반영 → 2차 제출"]
    C --> D{"합의 완료?"}
