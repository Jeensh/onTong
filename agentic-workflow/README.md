...
```
Phase 0: 플랜 협의                Phase 1: 반복 실행              Phase 2: 데모-피드백
┌─────────────────────┐      ┌──────────────────────┐      ┌─────────────────────┐
│                     │      │                      │      │                     │
│  요구사항 설명       │      │  CHANGES.md 확인     │ ←──── 피드백 → CHANGES.md  │
│       ↓             │      │       ↓              │      │                     │
│  AI: 플랜 초안 작성  │      │  코드 구현 (+ TDD)   │      │  사용자 데모 테스트   │
│       ↓             │      │       ↓              │      │       ↑             │
│  ★ 사용자 리뷰/편집  │ ───→ │  검증 + 문서 갱신    │ ───→ │  demo_guide 전달    │
│  (삭제/순서변경/축소) │      │       ↓              │      │                     │
│       ↓             │      │  TODO.md [x] 체크    │      │  Ad-hoc 변경 발생 시 │
│  TODO.md 확정        │      │       ↓              │      │  → 같은 턴에 문서    │
│                     │      │  사용자에게 보고      │      │    동기화 강제       │
└─────────────────────┘      └──────────────────────┘      └─────────────────────┘
```
...
```
1. AI가 master_plan 초안 제출 (스텝/태스크 표)
     ↓
2. 사용자가 표를 읽으며 편집
   - 태스크 삭제: "이건 지금 안 해"
   - 순서 변경: "이걸 먼저 해야 해"
   - 스텝 분할/병합: "이건 너무 크다" / "이건 합쳐도 돼"
   - 기술 선택 변경: "이건 X 대신 Y로 하자"
     ↓
3. AI가 피드백 반영해서 2차 제출
     ↓
4. 합의 → TODO.md 확정
```
